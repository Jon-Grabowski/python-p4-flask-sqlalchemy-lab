...
'''
    ...
    if not animals:
        response_body += f'<ul>No animals assigned to {zookeeper.name}</ul>'
    else:
...
def enclosure_by_id(id):
    ...
    if not enclosure:
        response_body = f'<ul>404 enclosure not found</ul>'
        response = make_response(response_body, 404)
        return response
...
'''
    ...
    if not animals:
        response_body += f'No animals in {enclosure.environment} environment'
